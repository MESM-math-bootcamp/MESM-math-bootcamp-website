...
[Day 1: Bren Math Workshop]{.custom-title}
...
::: {.body-text-l}
- **Day 1:** Algebra refresher; Rules of exponents
- **Day 2:** Units; Functions; Graphing; 
- **Day 3:**  Exponential functions; Logarithms; Linear functions
- **Day 4:**  Limits & Derivatives 
...
- Focus on applications to environmental science
- Are specifically relevant for MESM projects, coursework
...
3) A watershed's water balance equation is: 

:::{.center-text}
change in storage per unit time = precipitation ($P$) − actual evapotranspiration (ET) − river outflows ($Q$). 
:::

At steady state (storage isn't changing), actual evapotranspiration (ET) is 60% of precipitation and river outflow is $320$ mm/y. Solve for precipitation. 
:::
...
**3)** A watershed's water balance is: change in storage per unit time = precipitation ($P$) − actual ET − river outflows ($Q$). At steady state (storage isn't changing), with actual evapotranspiration (ET) being 60% of precipitation and $Q=320$ mm/y, solve for $P$.
...
(3) Any non-zero number to the power of 0 equals 1:
...
(3) Any non-zero number to the power of 0 equals 1: $x^0 = 1, \ \ \text{for } x \neq 0$
...
## {#fundamental-blank data-menu-title="Fundamental Theorem of Algebra"}

[Roots of polynomials]{.slide-title}
...
Consider the function $x - 2$, this is just a 1st degree polynomial. It has a single root, $r_1=2$. 

---

## {#example-second-degree-1 data-menu-title="Example: second degree"}
...
:::{style="color:green;"}
✏️ Check: What happens if $x=4$ or $x=-3$ in  $x^2-x-12$?
:::
...
## {#example-second-degree-2 data-menu-title="Example: second degree"}
...
✏️  Check: Verify the factorization on the right. 

---

## {#quad-blank data-menu-title="Quadratic Formula"}

[Quadratic Formula finds roots of 2nd degree polynomials]{.slide-title}
...
::: {.column width="60%"}
:::{style="color:green;"}
✏️  Example: Use the quadratic formula to find the solutions to $x^2-x-12=0$.
:::
:::
...
## {#quad-solution data-menu-title="Quadratic Formula: Solution"}

[Quadratic Formula finds roots of 2nd degree polynomials]{.slide-title}

<hr>

Solve $x^2-x-12=0$, where $a=1$, $b=-1$, $c=-12$:

$$
\begin{aligned}
x &= \frac{-b\pm\sqrt{b^2-4ac}}{2a} \\
&= \frac{-(-1)\pm\sqrt{(-1)^2-4(1)(-12)}}{2(1)} \\
...
So the roots are $x_1 = \frac{1+7}{2}=4$ or $x=\frac{1-7}{2}=-3$. This matches the roots we found earlier by factoring.
...
## {#esm-201-practice-blank data-menu-title="Rules of exponents: practice"}

[Population growth at discrete times]{.slide-title}
...
## {#esm-201-practice data-menu-title="Rules of exponents: practice" .smaller}
...
::: {.body-text-s}
A certain population size is measured at discrete time steps. We use the notation:
...
:::{style="color:green;"}
(1) Simplify this expression by factoring out $N_t$ and subsituting $\lambda = 1 + b -d$. 
...
(6) Explain in your own words what is the role of $\lambda$ in this model of population growth.
:::

---

## {#esm-201-practice-sols1 data-menu-title="Solutions 1"}
...
## {#esm-201-practice-sols2 data-menu-title="Solutions 2"}
...
## {#esm-201-practice-full-solutions data-menu-title="Full Solutions"}

[Population growth at discrete times]{.slide-title}

<hr>

**1)** $N_{t+1}=N_t(1+b-d)=\lambda N_t$

**2)** $N_1=\lambda N_0,\ N_2=\lambda^2 N_0,\ N_3=\lambda^3 N_0$

**3)** $N_T=\lambda^T N_0$

**4)** $\lambda = 1$

**5)** $\lambda>1$ grows (e.g. $\lambda=1.1$); $0\le\lambda<1$ shrinks (e.g. $\lambda=0.9$)

**6)** Each time step multiplies $N_t$ by $\lambda$, so $\lambda$ *is* the finite (discrete-time) growth factor.
