...
\begin{array}{|l|c|c|c|}
\hline
\textbf{Prefix} & \textbf{Symbol} & \textbf{Factor} & \textbf{Example} \\
\hline
\text{kilo} & \text{k} & 10^{3} & \text{km, kg, kL} \\
\hline
\text{hecto} & \text{h} & 10^{2} & \text{hm, hg, hL} \\
\hline
\text{deka} & \text{da} & 10^{1} & \text{dam, dag, daL} \\
\hline
\textit{(base unit)} & - & 10^{0} & \text{m, g, L} \\
\hline
\text{deci} & \text{d} & 10^{-1} & \text{dm, dg, dL} \\
\hline
\text{centi} & \text{c} & 10^{-2} & \text{cm, cg, cL} \\
\hline
\text{milli} & \text{m} & 10^{-3} & \text{mm, mg, mL} \\
\hline
\end{array}
$$

::: {.body-text-s}
The **factor** means how many times to multiply the base unit by 10. For example, 1 km = $10^3$ m = 1000 m, and 1 cm = $10^{-2}$ m = 0.01 m.
:::
...
**Example.** Convert 4.7 km into cm.
