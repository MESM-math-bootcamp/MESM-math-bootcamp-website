...
[**Exercise.** Convert 4.7 km into cm.]{style="color:green;"}
...
## {#ppm-blank data-menu-title="Parts per million (ppm)"}
...
<span style="color: red;">To convert **percent &rarr; ppm**, multiply percentage by 10,000. </span>

<span style="color: red;">To convert **ppm &rarr; percent**, divide ppm by 10,000.</span>
...
What does the $y$-intercept of $\frac{1}{N}\frac{dN}{dt}$ represent? What does its $x$-intercept represent?
:::
...
- $y$-intercept = the population's maximum per-capita growth rate when it is small and not yet resource-limited. This is known as the *intrinsic growth rate* $r$.

-  $x$-intercept = the population size at which per-capita growth stops. This is known as the *carrying capacity* $K$.
...
## {#reading-graphs-1 data-menu-title="Graphs explanation"} 
...
```{r, echo=FALSE, fig.width=6, fig.height=5, out.width="60%", fig.align="center"}
t <- seq(0, 10, length.out = 200)
pop <- 100 / (1 + 50 * exp(-2 * t))
logistic_df <- data.frame(time = t, population = pop)

ggplot(logistic_df, aes(x = time, y = population)) +
  geom_line(linewidth = 2, color = "slateblue") +
  labs(x = "Time (years)", y = "Population size (individuals)") +
  theme_classic() +
...
[💬 [Partner 1: read the graph to your partner.]{style="color:green;"}]{.slide-title3}
...
![](images/co2-emissions-per-capita.png){width="90%" fig-align="center"}

---

## {#graphs-reading-practice-2 data-menu-title="Graphs explanation"} 

[💬 [Partner 2: read the graph to your partner.]{style="color:green;"}]{.slide-title3}
<hr>
...
![](images/population.png){ fig-align="center" width="90%" }
...
- <span style="color:#E47200"><b>🧩 Definition of functions </b></span>  
...
- <span style="color:#808000"><b>🐟 Applications to MESM courses</b></span>  
